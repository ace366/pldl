<x-app-layout>
    <div class="py-6">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-4">
                <h1 class="text-xl font-semibold text-gray-800">拠点を編集</h1>
            </div>

            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('admin.bases.update', $base) }}">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-sm font-medium text-gray-700">拠点名</label>
                        <input type="text" name="name" value="{{ old('name', $base->name) }}" required
                    ...
                    <div class="mt-4">
                        {{-- ✅ OFFの時に値が送られない問題対策：必ず0を送る --}}
                        <input type="hidden" name="is_active" value="0">

                        <div class="flex items-center gap-2">
                            <input id="is_active" type="checkbox" name="is_active" value="1"
                                @checked(old('is_active', $base->is_active))
